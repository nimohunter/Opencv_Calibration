...
int main()
{
	...
	for (int i=1; i<=20; i++) {
        ...
        str << "/Your Cehssbord Picture path/chessboard" << std::setw(2) << std::setfill('0') << i << ".jpg";
    ...
    cv::Mat cameraDistortion_coeff = cameraCalibrator.getDistCoeffs();
    std::cout << "Camera distortion_coeff: " << cameraDistortion_coeff.rows << "x" << cameraDistortion_coeff.cols << std::endl;
    for (int i = 0; i < cameraDistortion_coeff.cols; i++)
        std::cout << cameraDistortion_coeff.at<double>(0, i) << " ";
    std::cout << std::endl;
